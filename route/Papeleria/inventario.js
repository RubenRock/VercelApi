...
const Inventario = require('../../models/Papeleria/Inventario')

const router = Router()

//obtener un dato
router.get('/api/papel_inventario/:id', async (req, res) => {
    let {id} = req.params
...
router.get('/api/papel_inventario', (req, res) => {
    Inventario.find()
    .exec()
    .then(x => res.status(200).send(x))
...
router.post('/api/papel_inventario', async (req, res) => {
    await Inventario.insertMany(req.body)    
    res.json({'message':'Saved successful', "data":req.body})  
})

router.delete('/api/papel_inventario/:id',async (req, res) => {
    let { id } = req.params;
    try{        
...
router.delete('/api/papel_inventario/',async (req, res) => {
    try{        
...
router.put('/api/papel_inventario/:id',async (req, res) => {
    let {id} = req.params
