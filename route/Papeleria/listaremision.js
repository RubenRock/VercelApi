...
const Listaremision = require('../../models/papeleria/Listaremision')


const router = Router()

//obtener un dato
router.get('/api/papel_listaremision/:id', (req, res) => {    
        let {id} = req.params
...
router.get('/api/papel_listaremision', (req, res) => {       
        Listaremision.find()
        .exec()
        .then(x => res.status(200).send(x))
...
router.post('/api/papel_listaremision',  async (req, res) => {   
...
router.delete('/api/papel_listaremision/:id',async (req, res) => {
    let { id } = req.params;
    try{        
...
router.delete('/api/papel_listaremision/',async (req, res) => {
    try{        
...
router.put('/api/papel_listaremision/:id',async (req, res) => {
    let {id} = req.params
