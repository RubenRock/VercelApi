...
const Remision = require('../../models/papeleria/Remisiones')

const router = Router()


//obtener un dato
router.get('/api/papel_remisiones/:id', (req, res) => {
    let {id} = req.params
...
router.get('/api/papel_remisiones', (req, res) => {
    Remision.find()
    .exec()
    .then(x => res.status(200).send(x))
...
router.post('/api/papel_remisiones', async (req, res) => {
    try{
...
router.delete('/api/papel_remisiones/:id',async (req, res) => {
    let { id } = req.params;
    try{        
...
router.delete('/api/papel_remisiones/',async (req, res) => {
    try{        
...
router.put('/api/papel_remisiones/:id',async (req, res) => {
    let {id} = req.params
